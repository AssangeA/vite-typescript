...
function useSfcTemplateScript(getUnreactiveDoc, template, styles, templateScriptData, styleDocuments, styleSourceMaps, templateData, sfcTemplateCompileResult, sfcStyles, scriptLang, context) {
    let version = 0;
    const _vueDoc = getUnreactiveDoc();
    const vueUri = _vueDoc.uri;
    const vueFileName = upath.basename(shared.uriToFsPath(_vueDoc.uri));
    const cssModuleClasses = reactivity_1.computed(() => styleDocuments.value.reduce((map, style) => {
        if (style.module) {
    ...
    const templateCodeGens = reactivity_1.computed(() => {
        ...
        if (!((_a = sfcTemplateCompileResult.value) === null || _a === void 0 ? void 0 : _a.ast))
            return;
        return templateGen.generate(templateData.value.sourceLang, sfcTemplateCompileResult.value.ast, context.isVue2Mode, templateScriptData.components, templateScriptData.setupReturns, [...cssScopedClasses.value.values()].map(map => [...map.keys()]).flat(), templateData.value.htmlToTemplate);
    });
    ...
    const cssSourceMap = reactivity_1.computed(() => {
        if (templateCodeGens.value && cssTextDocument.value && template.value) {
            const vueDoc = getUnreactiveDoc();
            const sourceMap = new SourceMaps.CssSourceMap(vueDoc, cssTextDocument.value.textDocument, cssTextDocument.value.stylesheet, undefined, false, [], { foldingRanges: false, formatting: false }, templateCodeGens.value.cssCodeGen.getMappings(parseMappingSourceRange));
            return sourceMap;
        }
    });
