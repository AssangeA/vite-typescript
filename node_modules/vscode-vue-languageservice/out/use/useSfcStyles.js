...
function useSfcStyles(context, getUnreactiveDoc, styles) {
    ...
    const textDocuments = reactivity_1.computed(() => {
        ...
        for (let i = 0; i < styles.value.length; i++) {
            ...
            documents.push({
                ...
                stylesheet,
                binds: stylesheet ? cssBinds_1.parse(content, stylesheet) : [],
                links: linkStyles,
                module: style.module,
                scoped: style.scoped,
            });
    ...
    const sourceMaps = reactivity_1.computed(() => {
        ...
        for (let i = 0; i < styles.value.length && i < textDocuments.value.length; i++) {
            ...
            const style = styles.value[i];
            const sourceMap = new SourceMaps.CssSourceMap(vueDoc, cssData.textDocument, cssData.stylesheet, style.module, style.scoped, cssData.links, { foldingRanges: true, formatting: true });
            sourceMap.add({
                data: undefined,
