    ...
    return (document, positions) => {
        ...
        function getTsResult(sourceFile) {
            ...
            for (const sourceMap of tsSourceMaps) {
                ...
                const tsStarts = positions.map(position => { var _a; return (_a = sourceMap.getMappedRange(position)) === null || _a === void 0 ? void 0 : _a.start; }).filter(shared.notEmpty);
                const tsSelectRange = dummyTs.ls.getSelectionRanges(dummyTs.uri, tsStarts);
                result = result.concat(transforms_1.transformSelectionRanges(tsSelectRange, range => sourceMap.getSourceRange(range.start, range.end)));
            }
            return result;
        }
        function getHtmlResult(sourceFile) {
            ...
            for (const sourceMap of [
                ...
                const htmlStarts = positions.map(position => { var _a; return (_a = sourceMap.getMappedRange(position)) === null || _a === void 0 ? void 0 : _a.start; }).filter(shared.notEmpty);
                ...
                result = result.concat(transforms_1.transformSelectionRanges(selectRanges, range => sourceMap.getSourceRange(range.start, range.end)));
            }
            return result;
        }
        function getCssResult(sourceFile) {
            ...
            for (const sourceMap of sourceFile.getCssSourceMaps()) {
                ...
                if (!cssLs || !sourceMap.stylesheet)
                    continue;
                const cssStarts = positions.map(position => { var _a; return (_a = sourceMap.getMappedRange(position)) === null || _a === void 0 ? void 0 : _a.start; }).filter(shared.notEmpty);
                const cssSelectRanges = cssLs.getSelectionRanges(sourceMap.mappedDocument, cssStarts, sourceMap.stylesheet);
                result = result.concat(transforms_1.transformSelectionRanges(cssSelectRanges, range => sourceMap.getSourceRange(range.start, range.end)));
            }
            return result;
        }
    };
